...
@HostPlugin(revision = "$Revision: 30628 $", interfaceVersion = 2, names = { "divshare.com" }, urls = { "https?://(www\\.)?divshare\\.com/(?:download|image|direct)/\\d+\\-[a-z0-9]+" }, flags = { 0 })
public class DivShareCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        // handling für die links, die "direct" beinhalten (diese starten sofort)
        if (br.getURL().contains("direct")) {
            ...
            try {
                ...
                br.followConnection();
                if (br.containsHTML("This file is unavailable until")) {
                    throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE);
                }
                if (br.containsHTML("Sorry, we couldn't find this file\\.|This file is secured by Divshare\\.")) {
            ...
            // handling für die anderen Links (mit "download" bzw. "image" im Link
            if (br.containsHTML("Sorry, we couldn't find this file\\.|This file is secured by Divshare\\.")) {
                throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
            }
            ...
            if (filename == null) {
                filename = br.getRegex("<div class=\"fileNameHeader\">([^<>\"]+)</div>").getMatch(0);
                if (filename == null) {
                    filename = br.getRegex("<title>([^<>\"]+) - DivShare</title>").getMatch(0);
                    if (filename == null) {
                        throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
                    }
                }
            ...
            if (id != null) {
                br2.postPage("http://www.divshare.com/scripts/ajax/v5/fileStats.php", "code=" + id);
            }
    ...
    public void handleFree(final DownloadLink link) throws Exception {
        ...
        if (br.containsHTML("Cannot connect to the rep database because:")) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Cannot connect to the rep database because:", 10 * 60 * 1000l);
        }
        br.setFollowRedirects(true);
        if (br.getURL().contains("direct")) {
            String dllink;
            ...
            // Check ob ein Passwort verlangt wird, wenn keins verlangt wird wird der Link sofort geändert und geladen (unten ists dann
            // infolink2)!
            ...
            getPage(infolink2);
            String dllink = br.getRegex("(https?://[a-z0-9]+\\.divshare\\.com/launch[^<>\"]+)\"").getMatch(0);
            if (dllink == null && br.containsHTML("application/mp3")) {
                dllink = br.getRegex("var ch1  = escape\\('(https?://[^<>']+)'\\);").getMatch(0);
            }
            if (dllink == null) {
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
            }
