...
@HostPlugin(revision = "$Revision: 31260 $", interfaceVersion = 2, names = { "filer.net" }, urls = { "https?://(www\\.)?filer\\.net/(get|dl)/[a-z0-9]+" }, flags = { 2 })
public class FilerNet extends PluginForHost {
    ...
    private void handleFreeErrorsAPI() throws PluginException {
        if (statusCode == 501) {
            ...
            throw new PluginException(LinkStatus.ERROR_HOSTER_TEMPORARILY_UNAVAILABLE, "Max free simultan-downloads-limit reached, please finish running downloads before starting new ones!", 2 * 60 * 1000l);
        }
        // 203 503 wait
        int wait = getWait();
        if (statusCode == 503) {
