...
@HostPlugin(revision = "$Revision: 26268 $", interfaceVersion = 2, names = { "youtube.com" }, urls = { "((httpJDYoutube|youtubeJDhttps?)://[\\w\\.\\-]*?(youtube|googlevideo)\\.com/(videoplayback\\?.+|get_video\\?.*?video_id=.+\\&.+(\\&fmt=\\d+)?))|((httpJDYoutube|youtubeJDhttps?)://[\\w\\.\\-]*?(video\\.google|googlevideo|youtube)\\.com/api/timedtext\\?.+\\&v=[a-z\\-_A-Z0-9]+)|((httpJDYoutube|youtubeJDhttps?)://img\\.youtube.com/vi/[a-z\\-_A-Z0-9]+/(hqdefault|mqdefault|default|maxresdefault)\\.jpg)" }, flags = { 2 })
public class Youtube extends PluginForHost {
    ...
    protected ConfigEntry mp3ConfigEntry;
    protected ConfigEntry bestEntryConfig;
    ...
    protected void setConfigElements() {
        ...
        getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_SEPARATOR));
        final StringBuilder sb = new StringBuilder();
        sb.append("<html>Explanation of the available tags:<br>");
        sb.append("*channelname* = uploaders/channels text name (nice name, allows long names & spaces)<br>");
        sb.append("*username* = uploaders/channels user name (youtube.com/users/example = example)<br>");
        sb.append("*date* = date when the video was posted - appears in the user-defined format above<br>");
        sb.append("*videoname* = name of the video without extension<br>");
        sb.append("*videoid* = id of the video: 'youtube.com/watch?v=YOUTUBE_VIDEO_ID'<br>");
        sb.append("*videonumber* = number of the video (position in playlist or 'user' link)<br>");
        sb.append("*quality* = the quality of the video e.g. '(1080p_H.264-AAC)'<br>");
        ...
        getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_SEPARATOR));
        getConfig().addEntry(mp3ConfigEntry = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX, getPluginConfig(), ALLOW_MP3, JDL.L("plugins.hoster.youtube.checkmp3", "Grab MP3?")).setDefaultValue(true));
        ...
        getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_CHECKBOX, getPluginConfig(), ALLOW_ORIGINAL, JDL.L("plugins.hoster.youtube.check2160p", "Grab 2160p?")).setDefaultValue(true));
        getConfig().addEntry(bestEntryConfig = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX, getPluginConfig(), ALLOW_BEST, JDL.L("plugins.hoster.youtube.checkbest", "Returns the best videos within your speficied Resolution and Format selections above!")).setDefaultValue(false));
        getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_SEPARATOR));
