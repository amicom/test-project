...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 2, names = { "mooshare.biz" }, urls = { "https?://(www\\.)?mooshare\\.biz/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class MooShareBiz extends PluginForHost {

    private String               correctedBR                  = "";
    private String               passCode                     = null;
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    // primary website url, take note of redirects
    private static final String  COOKIE_HOST                  = "http://mooshare.biz";
    // domain names used within download links.
    private static final String  DOMAINS                      = "(mooshare\\.biz)";
    ...
    public MooShareBiz(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|>Video Not Found or Deleted)").matches()) {
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null && VIDEOHOSTER_2) {
            try {
                ...
                final String embed_access = "http://" + COOKIE_HOST.replace("http://", "") + "/embed-" + fileid + ".html";
                getPage(embed_access);
                getPage(embed_access + "?play=1&confirm=Close+Ad+and+Watch+as+Free+User");
                dllink = getDllink();
                if (dllink == null) {
