...
@HostPlugin(revision = "$Revision: 30457 $", interfaceVersion = 2, names = { "video44.net" }, urls = { "http://(www\\.)?video44\\.net/gogo/\\?.+" }, flags = { 0 })
public class VideoFourtyFourNet extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (br.containsHTML("The file does not exist|\"video not found\"|Content has been removed due to copyright or from users")) {
        ...
        dllink = br.getRegex("file\\s*:\\s*('|\")(http.*?)\\1").getMatch(1);
        if (dllink == null) {
            dllink = br.getRegex("url\\s*:\\s*('|\")(http://.*?\\.(?:mp4|flv).*?)\\1").getMatch(1);
            if (dllink == null) {
                dllink = br.getRegex("url\\s*=\\s*('|\")(http://.*?\\.(?:mp4|flv).*?)\\1").getMatch(1);
            }
            if (dllink == null) {
