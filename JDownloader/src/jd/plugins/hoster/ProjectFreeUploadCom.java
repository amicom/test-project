...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 3, names = { "project-free-upload.com" }, urls = { "https?://(www\\.)?project-free-upload\\.com/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class ProjectFreeUploadCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://project-free-upload.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(project-free-upload\\.com)";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (cbr.containsHTML("(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|<li>The file (expired|deleted by (its owner|administration))|<h3>404 file not found or has been deleted</h3>)")) {
    ...
    public void correctDownloadLink(final DownloadLink downloadLink) {
        ...
        // set browser url to line up with correct url.
        try {
        ...
        if (fuid != null) {
            try {
                // set linkid
                downloadLink.setLinkID(this.getHost() + "://" + fuid);
            } catch (final Throwable t) {
            }
        }
    }
