...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 2, names = { "free4share.de" }, urls = { "https?://(www\\.)?free4share\\.de/[A-Za-z0-9]+" }, flags = { 2 })
public class Free4ShareDe extends PluginForHost {

    public Free4ShareDe(PluginWrapper wrapper) {
        super(wrapper);
    ...
    /* Basic constants */
    private final String         mainpage                                     = "http://free4share.de";
    private final String         domains                                      = "(free4share\\.de)";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            if (br.getURL().contains("/error." + type) || br.getURL().contains("/index." + type) || (!br.containsHTML("class=\"downloadPageTable(V2)?\"") && !br.containsHTML("class=\"download\\-timer\"") && !br.containsHTML("free4share\\.de/plugins/mediaplayer/site/_embed\\.php"))) {
                throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
            }
    ...
    private void login(final Account account, boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                br.postPage("http://free4share.de/ajax/_account_login.ajax.php", "username=" + Encoding.urlEncode(account.getUser()) + "&password=" + Encoding.urlEncode(account.getPass()));
                final String lang = System.getProperty("user.language");
                if (br.containsHTML("\"login_status\":\"invalid\"")) {
                ...
                br.getPage("/account_edit.html");
                if (!br.containsHTML("class=\"badge badge\\-success\">BEZAHLTER NUTZER<")) {
