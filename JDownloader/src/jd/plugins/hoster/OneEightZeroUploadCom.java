...
@HostPlugin(revision = "$Revision: 31155 $", interfaceVersion = 3, names = { "180upload.com" }, urls = { "https?://(www\\.)?180upload\\.(com|nl)/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 0 })
@SuppressWarnings("deprecation")
public class OneEightZeroUploadCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://180upload.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(180upload\\.com)";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (cbr.containsHTML("(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|<li>The file (expired|deleted by (its owner|administration))|>File Not Found, Copyright infringement issue, file expired or deleted by its owner\\.<)")) {
        ...
        if (!fuid.equalsIgnoreCase(fileInfo[0].trim())) {
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[1])) {
            ...
            if (inValidate(fileInfo[1])) {
                ...
                if (inValidate(fileInfo[1])) {
                    fileInfo[1] = br.getRegex("(\\d+(\\.\\d+)? ?(KB|MB|GB))").getMatch(0);
                    if (inValidate(fileInfo[1])) {
                        try {
