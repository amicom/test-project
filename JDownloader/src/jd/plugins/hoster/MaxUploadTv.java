...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 2, names = { "maxupload.tv" }, urls = { "http://(www\\.)?maxupload\\.tv/(file|e)/[A-Za-z0-9]+" }, flags = { 0 })
public class MaxUploadTv extends PluginForHost {

    public MaxUploadTv(PluginWrapper wrapper) {
        super(wrapper);
    ...
    // mods: huge changes, DO NOT UPGRADE!!
    // limit-info: Normal handling untested
    // protocol: no https
    ...
    /* Other constants */
    private final String         MAINPAGE                                     = "http://maxupload.tv";
    ...
    private static final String  TYPE_EMBED                                   = "http://(www\\.)?maxupload\\.tv/e/[A-Za-z0-9]+";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (link.getDownloadURL().matches(TYPE_EMBED)) {
            ...
            filename = br.getRegex("target=\"_blank\"><strong>([^<>\"]*?)</strong>").getMatch(0);
            if (filename == null) {
            ...
            if (filename == null) {
                filename = br.getRegex("<h2 style=\"text-align:center;\"> ([^<>\"]*?) \\(\\d+ ").getMatch(0);
            }
    ...
    public void doFree(final DownloadLink downloadLink, final String directlinkproperty, boolean resume, int maxchunks) throws Exception, PluginException {
        boolean captcha = false;
        if (downloadLink.getDownloadURL().matches(TYPE_EMBED)) {
            resume = true;
            maxchunks = 0;
            br.postPage(br.getURL(), "ok=yes&confirm=Close+Ad+and+Watch+as+Free+User&submited=true");
            String finallink = br.getRegex("url: \\'(http[^<>\"]*?)\\'").getMatch(0);
            if (finallink == null) {
                finallink = br.getRegex("\'(https?://(www\\.)?maxupload\\.tv:\\d+/file/[^<>\"]*?)\\'").getMatch(0);
            }
            if (finallink == null) {
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
            }
            ...
            if (continue_link != null) {
                ...
                for (int i = 1; i <= 3; i++) {
                    ...
                    } else if (waittime != null && i > 1) {
                        logger.info("Server error: Stuck in waittime loop, cannot start download");
                        throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error: Wait time loop", 3 * 60 * 60 * 1000l);
                    } else {
