...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 3, names = { "ryushare.com" }, urls = { "https?://(www\\.)?ryushare\\.com/((vid)?embed\\-)?[a-z0-9]{10,12}" }, flags = { 2 })
...
public class RyuShareCom extends PluginForHost {
    ...
    private boolean                    useAltLinkCheck              = true;
    ...
    private static final boolean       HOST_DOWN                    = false;
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (useAltLinkCheck) {
            ...
            setLinkCheckProperties(downloadLink, fileInfo);
    ...
    private void setLinkCheckProperties(final DownloadLink downloadLink, final String[] fileInfo) {
    ...
    private void doFree(final DownloadLink downloadLink, final Account account) throws Exception, PluginException {
        ...
        if (dl.getConnection().getContentType().contains("html")) {
            if (dl.getConnection().getResponseCode() == 503 && br.getHttpConnection().getHeaderField("server") != null && br.getHttpConnection().getHeaderField("server").toLowerCase(Locale.ENGLISH).contains("nginx")) {
                ...
                logger.warning("Final dllink seems not downloadable!");
                br.followConnection();
                correctBR();
                checkServerErrors();
                int timesFailed = downloadLink.getIntegerProperty("failedtimes_dllinunknowndlerror", 0);
                downloadLink.getLinkStatus().setRetryCount(0);
                if (timesFailed <= 2) {
                    timesFailed++;
                    downloadLink.setProperty("failedtimes_dllinunknowndlerror", timesFailed);
                    throw new PluginException(LinkStatus.ERROR_RETRY, "Final dllink seems not downloadable! Retrying...");
                } else {
                    downloadLink.setProperty("failedtimes_dllinunknowndlerror", Property.NULL);
                    logger.info("Final dllink has exausted retry count!");
                    throw new PluginException(LinkStatus.ERROR_FATAL, "Final dllink seems not downloadable! Exausted retry count!");
                }
            }
        } else {
    ...
    private void checkErrors(final DownloadLink theLink, final Account account, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        final String pr = "(Please Buy Premium To download this file<|>This file is available for Premium Users only\\.<|class=\"err\">Sorry! User who was uploaded this file requires premium to download\\.)";
        // let the fun begin!
    ...
    private void checkServerErrors() throws NumberFormatException, PluginException {
        ...
        if (cbr.containsHTML(">403 \\- Forbidden</")) {
        ...
        if (account.getBooleanProperty("free")) {
            ...
            if (inValidate(dllink)) {
                ...
                if (inValidate(dllink)) {
                    ...
                    if (br.containsHTML("class=\"err\">Skipped countdown</")) {
                        throw new PluginException(LinkStatus.ERROR_HOSTER_TEMPORARILY_UNAVAILABLE, "Server error", 1 * 60 * 1000l);
                    }
                    sendForm(dlform);
                    checkErrors(downloadLink, account, true);
                    getDllink();
                    ...
                    try {
                        ...
                        if (CrossSystem.isOpenBrowserSupported()) {
                            ...
                            if (JOptionPane.OK_OPTION == result) {
                                CrossSystem.openURL(new URL("http://ryushare.com/my-account.python"));
                            }
                        }
                    } catch (Throwable e) {
                    }
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception, PluginException {
        ...
        requestFileInformation(downloadLink);
        if (useAltLinkCheck) {
            // this will throw correct error exceptions for when dl routing kicks in. We need to use alt linkcheck because of multihoster
            // support.
            useAltLinkCheck = false;
            requestFileInformation(downloadLink);
        }
