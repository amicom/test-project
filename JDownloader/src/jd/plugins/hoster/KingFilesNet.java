...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 3, names = { "kingfiles.net" }, urls = { "https?://(www\\.)?kingfiles\\.net/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
...
public class KingFilesNet extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (cbr.containsHTML("(No such file|<h2>File Not Found<|>The file was removed by|Reason for deletion:\n|<li>The file (expired|deleted by (its owner|administration)))")) {
    ...
    private void doFree(final DownloadLink downloadLink, final Account account) throws Exception, PluginException {
        ...
        if (inValidate(dllink)) {
            ...
            if (download1 != null) {
                ...
                download1.remove("method_premium");
                sendForm(download1);
                if (br.getHttpConnection().getResponseCode() == 403 || br.containsHTML("<title>403 Forbidden</title>")) {
    ...
    public void correctBR() throws Exception {
        ...
        regexStuff.add("(<div class=\"foot\\d+\">.*?)<\\s*/BODY\\s*>");
        // generic cleanup
        regexStuff.add("<!(--.*?--)>");
