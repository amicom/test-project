...
@HostPlugin(revision = "$Revision: 29614 $", interfaceVersion = 2, names = { "remixshare.com" }, urls = { "http://[\\w\\.]*?remixshare\\.com/(.*?\\?file=|download/|dl/)[a-z0-9]+" }, flags = { 0 })
public class RemixShareCom extends PluginForHost {
    ...
    private String execJS() throws Exception {
        ...
        if (fun == null) {
            ...
            if (fun == null) {
                fun = br.getRegex("var \\w+\\s*(=\\s*\"((?:(?:https?:)?//(?:\\w+\\.)?remixshare\\.com)?/(?:[^/]+/){4})[^\r\n]+)").getMatch(0);
            }
        }
        if (fun == null) {
