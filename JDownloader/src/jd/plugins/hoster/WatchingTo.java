...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 2, names = { "watching.to" }, urls = { "https?://(www\\.)?watching\\.to/(embed\\-|watch/|external/)?[A-Za-z0-9]{12}" }, flags = { 0 })
public class WatchingTo extends PluginForHost {

    private String                         correctedBR                  = "";
    private String                         passCode                     = null;
    private static final String            PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    /* primary website url, take note of redirects */
    private static final String            COOKIE_HOST                  = "http://watching.to";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(watching\\.to)";
    ...
    private static final int               WAITSECONDSMIN               = 10;
    ...
    // protocol: no https
    // captchatype: null
    // other: watching.to/external/<fuid> == VIDEMBED_3
    ...
    public void correctDownloadLink(final DownloadLink link) {
        ...
        final String fid = new Regex(addedlink, "([a-z0-9]{12})$").getMatch(0);
        /* link cleanup, but respect users protocol choosing or forced protocol */
        if (SUPPORTSHTTPS || SUPPORTSHTTPS_FORCED) {
            link.setUrlDownload(COOKIE_HOST.replace("http://", "https://") + "/" + fid);
        } else {
    ...
    public WatchingTo(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|File Not Found|>The file expired|>It looks like that video no longer exist)").matches() || br.getHttpConnection().getResponseCode() == 404) {
