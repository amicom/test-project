...
@HostPlugin(revision = "$Revision: 31165 $", interfaceVersion = 2, names = { "megairon.net" }, urls = { "https?://(www\\.)?megairon\\.net/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class MegaIronNet extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|The file was deleted by administration)").matches()) {
    ...
    public void checkErrors(final DownloadLink theLink, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        if (new Regex(correctedBR, "(You have reached the download(\\-| )limit|You have to wait|>Delay between downloads must be not less than|>You have reached your daily downloads limit\\.<)").matches()) {
            ...
            if (tmpmin == null) {
                tmpmin = new Regex(correctedBR, " (\\d+) min").getMatch(0);
            }
        ...
        if (correctedBR.contains("You're using all download slots for IP")) {
            ...
            throw new PluginException(LinkStatus.ERROR_FATAL, "Downloads are disabled for your country");
        }
