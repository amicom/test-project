...
@HostPlugin(revision = "$Revision: 26673$", interfaceVersion = 2, names = { "brupload.net" }, urls = { "https?://(www\\.)?brupload\\.net/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class BrUploadNet extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://brupload.net";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(brupload\\.net)";
    private static final String            HTML_MAINTENANCE             = ">This server is in maintenance mode";
    private static final String            HTML_PREMIUMONLY             = ">Downloads are disabled for your user type";
    ...
    // mods:
    // limit-info: free/premium account untested, set FREE limits
    // protocol: http https
    ...
    public BrUploadNet(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, HTML_MAINTENANCE).matches()) {
            link.getLinkStatus().setStatusText(MAINTENANCEUSERTEXT);
            return AvailableStatus.UNCHECKABLE;
        }
        if (br.getURL().contains("/?op=login&redirect=") || correctedBR.contains(HTML_PREMIUMONLY)) {
