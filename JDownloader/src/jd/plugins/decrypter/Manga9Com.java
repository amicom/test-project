...
@DecrypterPlugin(revision = "$Revision: 28753 $", interfaceVersion = 2, names = { "manga9.com" }, urls = { "http://(www\\.)?manga9\\.com/[a-z0-9\\-_]+/\\d+/" }, flags = { 0 })
public class Manga9Com extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (br.getHttpConnection().getResponseCode() == 404 || br.containsHTML("is not available yet")) {
        ...
        String serverpart = br.getRegex("(http://z\\.mhcdn\\.net/store/manga/\\d+/[0-9\\-]+\\.0/compressed/[a-z]+(\\d+\\-)?)").getMatch(0);
        if (serverpart == null) {
