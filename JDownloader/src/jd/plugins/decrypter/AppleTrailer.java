...
@DecrypterPlugin(revision = "$Revision: 28206 $", interfaceVersion = 2, names = { "apple.com" }, urls = { "http://[\\w\\.]*?apple\\.com/trailers/(disney|dreamworks|entertainmentone|filmdistrict|focus_features|fox|fox_searchlight|independent|lions_gate|lucasfilm|magnolia|marvel|oscilloscope|paramount|picturehouse|relativity|sony|sony_pictures|summit|(universial|universal)|wb|(weinstein|weinstien))/[a-zA-Z0-9_]+/" }, flags = { 0 })
public class AppleTrailer extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (tryposter) {
            ...
            if (title == null) {
                title = br1.getRegex("<meta name=\"Keywords\" content=\"(.*?) Trailer").getMatch(0);
            }
            ...
            if (results != null && results.length != 0) {
                for (String result : results) {
                    if (dupe.add(br1.getURL() + result) == false) {
                        continue;
                    }
    ...
    private void processNormal() throws IOException, PluginException {
        ...
        if (title == null) {
            title = br2.getRegex("var trailerTitle = '(.*?)';").getMatch(0);
        }
        if (title == null) {
